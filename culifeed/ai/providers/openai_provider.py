...
OpenAI provider for GPT models with comprehensive error handling,
...
class OpenAIProvider(AIProvider):
    """OpenAI provider with async support and comprehensive error handling."""

    # OpenAI rate limits for gpt-4o-mini (Tier 1)
    DEFAULT_RATE_LIMITS = RateLimitInfo(
        requests_per_minute=500,
        requests_per_day=10000,  # 10,000 requests per day
        tokens_per_minute=200000,  # 200,000 tokens per minute
        tokens_per_day=None,  # No daily token limit
    )
    ...
    def __init__(self, api_key: str, model_name: str = "gpt-4o-mini"):
        ...
        super().__init__(api_key, model_name, AIProviderType.OPENAI)
        ...
        self.logger = get_logger_for_component("openai_provider")
        self._rate_limit_info = self.DEFAULT_RATE_LIMITS
        self._last_request_time = 0.0
        self._request_count_minute = 0
        self._minute_start = time.time()

        self.logger.info(f"OpenAI provider initialized with model: {model_name}")
    ...
    async def analyze_relevance(self, article: Article, topic: Topic) -> AIResult:
        ...
        try:
            ...
            raise AIError(
                f"Connection to OpenAI failed: {e}",
                provider="openai",
    ...
    async def test_connection(self) -> bool:
        ...
        try:
            ...
            if success:
                ...
                self.logger.warning(
                    "OpenAI connection test failed - unexpected response"
                )
    ...
    def set_model(self, model_name: str) -> None:
        ...
        if model_name in self.get_available_models():
            ...
            self.logger.info(f"Switched OpenAI model to: {model_name}")
        else:
            self.logger.warning(
                f"Unknown OpenAI model: {model_name}, keeping current: {self.model_name}"
            )
    ...
    def _handle_rate_limit_error(self, error) -> None:
        ...
        Args:
            error: Rate limit error from OpenAI API
        """
    ...
    def get_available_models() -> List[str]:
        ...
        """
        return [
            "gpt-4o-mini",  # Cost-effective, latest
    ...
    def __repr__(self) -> str:
        ...
        return f"OpenAIProvider(model={self.model_name}, rate_limit={self._request_count_minute}/{self.DEFAULT_RATE_LIMITS.requests_per_minute})"
